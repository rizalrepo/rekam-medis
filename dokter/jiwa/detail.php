        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h5 class="modal-title" id="custom-width-modalLabel"> <button type="button" class="btn btn-default waves-effect" data-dismiss="modal"><i class="fa fa-info-circle"></i></button> Detail Data Diagnosa Jiwa/Psikolog</h5>
                    <button type="button" class="btn btn-default waves-effect" data-dismiss="modal"><i class="fa fa-window-close"></i></button>
                </div>
                <?php
                $q = $con->query("SELECT * FROM mcu_jiwa m JOIN pasien p ON p.id_pasien = m.id_pasien JOIN dokter d ON d.id_dokter = m.id_dokter JOIN user u ON u.id_user = m.id_user WHERE id_mcu_jiwa = '$id' ");
                $d = $q->fetch_array();
                ...
                <div class="modal-body">
                    <div class="col-md-12">
                        <div class="form-group">
                            <?php if ($d['verif'] == 1) { ?>
                                <a href="<?= base_url() ?>/admin/jiwa/surat?id=<?= $id ?>" class="btn btn-sm btn-primary" target="_blank"> <i class="fa fa-print"></i> Cetak Surat</a>
                            <?php } ?>
                            <div class="card-body" style="text-align: left;">
                                ...
                                <dl class="row">
                                    ...
                                    <dt class="col-sm-3">Rohani</dt>
                                    <dd class="col-sm-9">: <?= $d['rohani'] ?></dd>
                                    <dt class="col-sm-3">Psikolog</dt>
                                    <dd class="col-sm-9">: <?= $d['psikolog'] ?></dd>
