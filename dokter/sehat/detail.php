        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h5 class="modal-title" id="custom-width-modalLabel"> <button type="button" class="btn btn-default waves-effect" data-dismiss="modal"><i class="fa fa-info-circle"></i></button> Detail Data Diagnosa Keterangan Sehat</h5>
                    <button type="button" class="btn btn-default waves-effect" data-dismiss="modal"><i class="fa fa-window-close"></i></button>
                </div>
                <?php
                $q = $con->query("SELECT * FROM mcu_sehat m JOIN pasien p ON p.id_pasien = m.id_pasien JOIN dokter d ON d.id_dokter = m.id_dokter JOIN user u ON u.id_user = m.id_user WHERE id_mcu_sehat = '$id' ");
                $d = $q->fetch_array();
                ...
                <div class="modal-body">
                    <div class="col-md-12">
                        <div class="form-group">
                            <?php if ($d['verif'] == 1) { ?>
                                <a href="<?= base_url() ?>/admin/sehat/surat?id=<?= $id ?>" class="btn btn-sm btn-primary" target="_blank"> <i class="fa fa-print"></i> Cetak Surat</a>
                            <?php } ?>
                            <div class="card-body" style="text-align: left;">
                                ...
                                <dl class="row">
                                    ...
                                    <dd class="col-sm-9">: <?= $d['tekanan'] ?> mmHg</dd>
                                    <dt class="col-sm-3">Tinggi Badan</dt>
                                    <dd class="col-sm-9">: <?= $d['tinggi'] ?> Cm</dd>
                                    <dt class="col-sm-3">Berat Badan</dt>
                                    <dd class="col-sm-9">: <?= $d['berat'] ?> kg</dd>
                                    <dt class="col-sm-3">Nadi</dt>
                                    <dd class="col-sm-9">: <?= $d['nadi'] ?> x / Menit</dd>
                                    <dt class="col-sm-3">Respirasi</dt>
                                    <dd class="col-sm-9">: <?= $d['respirasi'] ?> x / Menit</dd>
                                    <dt class="col-sm-3">Suhu Badan</dt>
                                    <dd class="col-sm-9">: <?= $d['suhu'] ?> &#8451;</dd>
